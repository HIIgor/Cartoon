...
class MineViewController: CTBaseViewController {
    ...
    private lazy var head: CTMineHeaderView = {
        return CTMineHeaderView(frame: CGRect(x: 0, y: 0, width: screenWidth, height: 200))
    }()
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(cellType: CTBaseTableViewCell.self)
        return tableView
    }()
    ...
    override func configNavBar() {
        ...
        navigationController?.setBarStyle(.clear)
...
extension MineViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView.contentOffset.y >= -(scrollView.parallaxHeader.minimumHeight) {
            navigationController?.setBarStyle(.theme)
