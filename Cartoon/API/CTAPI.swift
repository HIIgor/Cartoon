...
/// 网络请求开始和结束的拦截插件,用以添加loading
let loadinigPlugin = NetworkActivityPlugin { (type, target) in
...
let timeoutClosure = {
    (end: Endpoint, closure: MoyaProvider<CTApi>.RequestResultClosure) -> Void in
    if var urlRequest = try? end.urlRequest() {
        urlRequest.timeoutInterval = 20
        closure(.success(urlRequest))
    } else {
        closure(.failure(MoyaError.requestMapping(end.url)))
    }
}

let ApiProvider = MoyaProvider<CTApi>(requestClosure: timeoutClosure)
let ApiLoadingProvider = MoyaProvider<CTApi>(requestClosure: timeoutClosure, plugins:[loadinigPlugin])
